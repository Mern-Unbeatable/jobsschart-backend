 ...
 * Suppressed: Logs are noisy during startup
 */
...
export function startBillingTimer(conversationId) {
  ...
  const timer = setInterval(async () => {
    try {
      ...
      log.info(
        `⏰ [${conversationId}] elapsed=${Math.floor(elapsedSeconds)}s completed=${completedMinutes} billed=${alreadyBilled} → billing ${unbilled}`,
      );
...
export const initSocket = (httpServer) => {
  ...
  io.on('connection', (socket) => {
    // Socket connected - suppress verbose logging to keep terminal clean
    ...
    socket.on('disconnect', async () => {
      // Socket disconnected - suppress verbose logging
      if (socket.userId) {
        setTimeout(async () => {
    ...
    socket.on('send_message', async (data, callback) => {
      try {
        ...
        if (!isConsultant) {
          ...
          if (!wallet || parseFloat(wallet.creditBalance) < 0.1) {
        ...
        socket
          ...
          .emit('new_message', { conversationId, message: newMessage });
        const participants = await prisma.chatParticipant.findMany({
          where: { conversationId },
          select: { userId: true },
        });
    ...
    socket.on('send_file', async (data, callback) => {
      try {
        ...
        socket
          .to(`conv_${conversationId}`)
          .emit('new_file', { conversationId, message: newMessage });
        const participants = await prisma.chatParticipant.findMany({
          where: { conversationId },
          select: { userId: true },
        });
...
export async function notifyChatRequestPending(conversationId, extra = {}) {
  ...
  const payload = {
    ...
    pricePerMinute: extra.pricePerMinute || 2.5,
  };
  io.to(`conv_${conversationId}`).emit('chat_request_pending', payload);
  for (const p of participants) {
    io.to(`user_${p.userId}`).emit('chat_request_pending', payload);
